...
module.exports = function _export(relativeInputDirectory, relativeOutputDirectory, options) {
  ...
  gaze(path.join(relativeInputDirectory,'*.json'), function(error, watcher) {
    watcher.on('changed', function(filepath) {
      try {
        var locales = [filepath].map(getFileInfo).filter(validLocale);

        if ( ! locales.length) {
          return console.log(path.relative(process.cwd(), filepath)+' contains values that are not strings');
        }

        locales.forEach(exportLocale);
      } catch(error) {
        console.log('ERROR exporting '+path.relative(process.cwd(), filepath)+': '+error.message);
      }
    });
  });
